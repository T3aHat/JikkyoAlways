                ...
                if (exURL(results[j]) and (not longtxt(results[j]))):
                    if (rmwd):
                        nowdatalist[i].text = results[j].text.replace(
                            "\n", "").replace(word.replace(" -filter:retweets", ""), "")
                    else:
                        nowdatalist[i].text = results[j].text.replace(
                            "\n", "")
...
def Change_word(f):
    def x(self):
        ...
        bl6 = tkinter.BooleanVar(subf)
        bl6.set(rmwd)
        CheckBox6 = tkinter.Checkbutton(
            subf, text="remove Search word", variable=bl6)
        CheckBox6.pack()
...
def get_default(f, txt1, bl1, bl2, bl3, bl4, bl5, bl6, bl7, txt2, txt3, txt4, txt5, txt6, txt7):
    ...
    txt2.insert(tkinter.END, "15")
    txt3.delete(0, tkinter.END)
    txt3.insert(tkinter.END, "rec")
    ...
    for i in range(num_comment):
        if(i < len(nowdatalist)):
            if exURL(nowdatalist[i]) and (not longtxt(nowdatalist[i])):
                if(rmwd):
                    nowdatalist[i].text = nowdatalist[i].text.replace(
                        "\n", "").replace(word.replace(" -filter:retweets", ""), "")
                else:
                    nowdatalist[i].text = nowdatalist[i].text.replace("\n", "")
            else:
